...
const lockedProfileId = process.env.NEXTDNS_LOCKED_PROFILE_ID; // the profile we want to ensure is "locked"


if (!apiKey || !lockedProfileId) return res.status(500).json({ error: 'Missing env vars' });


// Try to activate the locked profile if the endpoint exists
try {
...
// Fallback: patch the locked profile to re-enable blocking
try {
const lockBody = {
settings: {
